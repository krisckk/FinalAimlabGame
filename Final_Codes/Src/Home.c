...
static void Init(void){
    ...
    Home -> Themesong = load_audio("Assets/Music/Intestellar.ogg");
    play_bgm(Home -> Themesong, music_volume);
...
static void Destroy(void){
    ...
    al_destroy_sample(Home -> Themesong);
    MCDestroy(MC);
...
static void On_key_down(int key_code) {
    ...
    if (Home -> staminaRestore){
        if(key_code == ALLEGRO_KEY_E){
            game_change_scene(scene_staminarestore_create());
        }
    }
    if (Home -> equipement){
        if(key_code == ALLEGRO_KEY_E){
